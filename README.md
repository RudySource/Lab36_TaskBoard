...
> **Точный порт можно посмотреть в** `TaskBoardApi/Properties/launchSettings.json`.
...
> [!WARNING]
> Важно: frontend нужно открывать после запуска backend, иначе список задач не загрузится❗️
...
> [!WARNING]
> Фуууух, вот и закончил. Жизнь — это маленькая кухня, на которой мы готовим блюда под названием „счастье“. На этой кухне мы сами себе шеф‑повара, и только нам решать, какие ингредиенты мы будем добавлять в наши блюда. Важно помнить, что универсального рецепта нет, поэтому творите, пробуйте что‑то новое. И, может быть, тогда в конце вас будет ждать награда
